...
public class EmployeeManagement {
    ...
    public ArrayList<Integer> timKiemViTriThiDau(String viTri){
        ...
        for(int i=0; i< listNVs.size(); i++)
            if(listNVs.get(i) instanceof Player && ((Player)listNVs.get(i)).getViTriThiDau().trim().equalsIgnoreCase(viTri.trim())){
                arr.add(i);
            }
        return arr;
    }
    ...
    private int indexByName(){
        ...
        // id ds nhan vien:
        for(int i=0; i< listNVs.size(); i++){
            if(listNVs.get(i) instanceof Player && listNVs.get(i).getTen().trim().equalsIgnoreCase(name.trim())){
                arrCT.add(i);
            }
            else if(listNVs.get(i) instanceof Coach && listNVs.get(i).getTen().trim().equalsIgnoreCase(name.trim())){
                arrHLV.add(i);
            }
        }
        ...
        if(arrCT.size() + arrHLV.size() ==1){
            ...
            do{
                ...
                try{
                    ...
                    if(index >-1){
                        tf= true;
                        if(listNVs.get(index) instanceof Player)
        ...
        do{
            ...
            switch(op){
                ...
                case 2:
                    ...
                    Coach hlv = this.nhapHuanLuyenVien();
                    if(this.them(hlv))
                        System.out.println("Them thanh cong");
                    else
    ...
    private Player nhapCauThu(){
        ...
        System.out.println("Ngay ký hợp đồng:");
        ct.setNgayVaoLam(this.convertStringToDate());
        System.out.println("Ví trí thi đấu:");
        ct.setViTriThiDau(sc.nextLine());
        System.out.println("Số trận tham gia:");
        ct.setSoLuotTranThamGia(this.inputInt());
        System.out.println("Số bàn thắng:");
    ...
    private Coach nhapHuanLuyenVien(){
        ...
        System.out.println("Số năm kinh nghiem:");
    ...
    private void nhapDuLieuTuDong(){
        Player ct;
        try {
            ct = new Player("Nguyen Van A","Viet Nam",true,new SimpleDateFormat("dd/MM/yyyy").parse("28/12/1994"),new SimpleDateFormat("dd/MM/yyyy").parse("10/1/2014")
                    ,"Tien dao",23,20,20000000);
            listNVs.add(ct);
            Coach hlv = new Coach("UHkw","Phap",false,new SimpleDateFormat("dd/MM/yyyy").parse("5/3/1980"),new SimpleDateFormat("dd/MM/yyyy").parse("2/4/2010")
                    , 10, 5.0, 3000000);
            listNVs.add(hlv);
            Player ct1 = new Player("Phan Van B","Viet Nam",true,new SimpleDateFormat("dd/MM/yyyy").parse("6/2/1994"),new SimpleDateFormat("dd/MM/yyyy").parse("12/3/2014")
                    ,"Hau ve",23,2,12000000);
            listNVs.add(ct1);
            Player ct2 = new Player("Tran Van Q","Viet Nam",true,new SimpleDateFormat("dd/MM/yyyy").parse("6/4/1994"),new SimpleDateFormat("dd/MM/yyyy").parse("2/1/2014")
                    ,"Trung ve",23,0,12400000);
            listNVs.add(ct2);
            ////
            Player ct3 = new Player("Phan van b","Sig",true,new SimpleDateFormat("dd/MM/yyyy").parse("12/2/1993"),new SimpleDateFormat("dd/MM/yyyy").parse("1/5/2010")
                    ,"Hau ve",3,4,12000000);
            listNVs.add(ct3);
            Player ct4 = new Player("uhkw","Thai",true,new SimpleDateFormat("dd/MM/yyyy").parse("20/2/1992"),new SimpleDateFormat("dd/MM/yyyy").parse("1/5/2010")
                    ,"Hau ve",12,4,22000000);
            listNVs.add(ct4);
            Coach hlv1 = new Coach("UHkw","Nga",false,new SimpleDateFormat("dd/MM/yyyy").parse("5/3/1980"),new SimpleDateFormat("dd/MM/yyyy").parse("2/4/2010")
                    , 10, 5.0, 3000000);
            listNVs.add(hlv1);
            Coach hlv2 = new Coach("UHkw","My",false,new SimpleDateFormat("dd/MM/yyyy").parse("5/3/1980"),new SimpleDateFormat("dd/MM/yyyy").parse("2/4/2010")
                    , 10, 5.0, 3000000);
            listNVs.add(hlv2);
    ...
    private void tbCauThu(ArrayList<Integer> arr){
        ...
        for (int i = 0; i < arr.size(); i++) {
            Player ct = (Player) listNVs.get(arr.get(i));
    ...
    private void tbHuanLuyenVien(ArrayList<Integer> arr){
        ...
        System.out.format(line);
        for (int i = 0; i < arr.size(); i++) {
            Coach hlv = (Coach) listNVs.get(arr.get(i));
    ...
    private void tbHuanLuyenVienByIndex(int index){
        Coach hlv = (Coach) listNVs.get(index);
        String leftAlignFormat = "| %-7s | %-13s | %-9s | %-9s | %-9s | %-13s | %-11s | %-11s | %-15s |%n";
        String line = "+---------+---------------+-----------+-----------+------------+---------------+-------------+-------------+-----------------+%n";
        System.out.format(line);
        System.out.format("| Index   | Ho va ten     | Quoc tich | Gioi tinh | Ngay sinh  | Ngay vao lam  | Kinh nghiem | He so luong | Phu cap         |%n");
    ...
    private void tbCauThuByIndex(int index){
        Player ct = (Player) listNVs.get(index);
        String leftAlignFormat = "| %-7s | %-13s | %-9s | %-9s | %-10s | %-13s | %-13s | %-7s | %-9s | %-16s |%n";
        String line = "+---------+---------------+-----------+-----------+------------+---------------+---------------+---------+-----------+------------------+%n";
        System.out.format(line);
        System.out.format("| Index   | Ho va ten     | Quoc tich | Gioi tinh | Ngay sinh  | Ngay vao lam  | Vi tri        | So tran | Ban thang | Luong thoa thuan |%n");
